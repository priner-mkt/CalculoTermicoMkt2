...
<style>
    ...
    .stSuccess, .stInfo, .stWarning, .stError { border-radius: 8px; padding: 1rem; }
...
# --- LÓGICA DE EXIBIÇÃO: FORMULÁRIO OU CALCULADORA ---

if not st.session_state.form_submitted:
    # --- SEÇÃO DO FORMULÁRIO ---
    st.header("Acesse a Calculadora")
    st.markdown("Preencha os campos abaixo para liberar o acesso à ferramenta e receber novidades.")
    ...
    with st.form(key="lead_form"):
        ...
        application_options = [
            "Isolamento Térmico Quente", "Isolamento Térmico Frio", 
            "Isolamento Fixo", "Isolamento Acústico", 
            "Proteção contra Intempéries/Vazamentos"
        ]
        ...
        if not user_name or not user_email or not user_application:
            ...
            with st.spinner("Enviando seus dados..."):
                success = enviar_conversao_rdstation(
                    ...
                    application_type=user_application
                )
